...
#define MAX_NUM_NODES 1000				//1000
	/*
	struct Node {
		int64_t dist;
		tableint ep_id;
	...
	*/
	...
	public:
		...
		std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst>
			...
			std::cout << "candidate_set_size: " << candidate_set_size << ", top_candidate_sets_size: " << top_candidates_set_size << std::endl;
...
#endif
		/*
		std::priority_queue<std::pair<dist_t, labeltype >> searchKnn_bf(const void* query_data, size_t k, size_t HW) const {
			tableint currObj = enterpoint_node_;
			dist_t curdist = fstdistfunc_(query_data, getDataByInternalId(enterpoint_node_), dist_func_param_);
			...
			StaticPriorityQueue top_candidates;
			if (has_deletions_) {
				StaticPriorityQueue top_candidates1 = searchBaseLayerST<true>(
					currObj, query_data, std::max(ef_, k));
				top_candidates.Swap(top_candidates1);
			}
			else {
				StaticPriorityQueue top_candidates1 = searchBaseLayerST<false>(
					currObj, query_data, std::max(ef_, k));
				top_candidates.Swap(top_candidates1);
			}
			std::priority_queue<std::pair<dist_t, labeltype >> results;
		...
		*/
		...
		StaticPriorityQueue searchKnn(const void* query_data, size_t k, size_t HW) const {
			tableint currObj = enterpoint_node_;
			dist_t curdist = fstdistfunc_(query_data, getDataByInternalId(enterpoint_node_), dist_func_param_);
